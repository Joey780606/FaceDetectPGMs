...
FaceRecognizer 類別：整合 face_recognition、face_feature 與 random_forest_np，
...
class FaceRecognizer:
    """
    人臉辨識器。
    以 face_recognition 偵測 68 個 landmark，
    ...
    def Predict(self, Frame: np.ndarray) -> list:
        ...
        try:
            ...
            ValidIdxs = []
            Vecs      = []
            for i, (Loc, Lm) in enumerate(zip(Locations, LandmarksList)):
                Vec = extractFeatures(Lm)
                if Vec is not None:
                    ValidIdxs.append(i)
                    Vecs.append(Vec)
            ...
            for j, OrigIdx in enumerate(ValidIdxs):
                Top, Right, Bottom, Left = Locations[OrigIdx]
