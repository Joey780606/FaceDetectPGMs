...
使用 MediaPipe FaceLandmarker + OneClassSVM 進行人臉辨識。
透過 CustomTkinter 建立 GUI，可學習並辨識不同人的身份。
五象限 SVM：正臉、左上、右上、左下、右下，各象限達 20 張後自動完成學習。
...
from face_recognizer import FaceRecognizer, POSE_NAMES, POSE_TARGET

# --- 應用程式常數 ---
UI_REFRESH_MS             = 30   # webcam 畫面更新間隔（毫秒）
LEARN_TICK_MS             = 500  # 學習時每次抓 frame 的間隔（每秒 2 張）
DETECT_TICK_MS            = 300  # 辨識時每次推論的間隔
DETECT_NONE_DETECT_TARGET = 5    # 多數決所需累積幀數
...
class MainApp(customtkinter.CTk):
    ...
    def _BuildUI(self) -> None:
        ...
        self.title("人臉辨識系統（MediaPipe 五象限 SVM）")
        ...
        self._BtnDetectNone = customtkinter.CTkButton(
            Row0Top, text="Detect", width=120, command=self._OnBtnDetectNone
        )
        self._BtnDetectNone.pack(side="left", padx=(5, 10), pady=5)

        self._LblDetectName = customtkinter.CTkLabel(
            Row0Top, text="", font=customtkinter.CTkFont(size=16, weight="bold")
        )
        ...
        self._BtnLearn = customtkinter.CTkButton(
            Row1Top, text="Learning", width=120, command=self._OnBtnLearn
        )
        ...
        self._LblPoseCounts = []
        for i, PoseName in enumerate(POSE_NAMES):
            self._Row1Bot.grid_columnconfigure(i, weight=1)
            Lbl = customtkinter.CTkLabel(
                self._Row1Bot,
                text=f"{PoseName}: 0/{POSE_TARGET}"
            )
            Lbl.grid(row=0, column=i, padx=8, pady=5, sticky="ew")
            self._LblPoseCounts.append(Lbl)
    ...
    def _InitComponents(self) -> None:
        ...
        if not CamOk:
            MsgBox.showwarning("攝影機錯誤", "無法開啟攝影機，請確認連線後重新啟動。\n應用程式將以無攝影機模式執行。")
    ...
    def _UpdateWebcamView(self) -> None:
        """從 WebcamManager 取得最新 frame，繪製快取的偵測框，轉換後顯示。"""
        try:
    ...
    def _OnBtnDetectNone(self) -> None:
        ...
        self._LblDetectName.configure(text=f"偵測中.... (0/{DETECT_NONE_DETECT_TARGET})")
        self.after(0, self._DetectNoneTick)

    def _DetectNoneTick(self) -> None:
        """DetectNone 辨識 tick，在背景執行緒執行推論，避免阻塞 UI。"""
    ...
    def _OnBtnLearn(self) -> None:
        ...
        # 若已完成全部象限，提示無需再訓練
        PoseCounts = self._Recognizer.GetLearnPoseCounts(PersonName)
        if all(PoseCounts.get(i, 0) >= POSE_TARGET for i in range(5)):
            MsgBox.showinfo(
                "已完成訓練",
                f"[{PersonName}] 已完成所有象限的訓練（各 {POSE_TARGET} 張）。\n"
                f"若要重新訓練，請先 Remove 後再訓練。"
            )
            return
    ...
    def _StartLearning(self, PersonName: str) -> None:
        ...
        # 顯示現有計數（支援多次訓練補足剩餘象限）
        PoseCounts = self._Recognizer.GetLearnPoseCounts(PersonName)
        for i, Lbl in enumerate(self._LblPoseCounts):
            Count = PoseCounts.get(i, 0)
            Lbl.configure(text=f"{POSE_NAMES[i]}: {Count}/{POSE_TARGET}")
        self._Row1Bot.grid()

        self._AppendLog(f"開始學習 [{PersonName}]，請對鏡頭慢慢做各角度")
    ...
    def _LearningTick(self) -> None:
        """學習模式每次 tick（每 500ms）：收集人臉樣本並更新象限進度。"""
                ...
                if Ok and Frame is not None:
                    ...
                    def Worker():
                        try:
                            ...
                            PoseCounts = self._Recognizer.GetLearnPoseCounts(PersonName)
                            self.after(0, lambda A=Added, KP=KeyPoints, PC=PoseCounts:
                                       self._OnLearnSampleAdded(A, KP, PC))
                        except Exception as Error:
    ...
    def _OnLearnSampleAdded(self, Added: bool, KeyPoints: list, PoseCounts: dict) -> None:
        """學習樣本加入回調（在主執行緒執行）。"""
        if not self._LearnActive:
            return
