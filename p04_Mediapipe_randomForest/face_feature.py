...
從 MpFaceDetector.detect() 回傳的 68 個 landmark 座標中，
萃取具代表性的臉部幾何特徵向量，供 Random Forest 訓練與辨識使用。

輸入格式與 face_recognition.face_landmarks() 相同（dict，key 為部位名稱），
因此此檔案與 p03 完全相容，無需修改核心邏輯。
...
def extractFeatures(Landmarks: dict) -> np.ndarray | None:
    """
    從 MpFaceDetector 回傳的單張臉 landmark dict 中，萃取 23 維特徵向量。

    Parameters
    ----------
    Landmarks : dict
        key 為部位名稱，value 為 (x, y) tuple 的列表。
        與 face_recognition.face_landmarks() 格式相同。
    ...
    try:
        ...
        # 嘴巴（top_lip 共 12 個點，index 0=左角，index 6=右角）
        # MediaPipe 映射：61=左嘴角，291=右嘴角，符合 face_recognition 格式
        ...
        # 下巴（chin 共 17 個點，index 0=左端，index 8=最低點，index 16=右端）
