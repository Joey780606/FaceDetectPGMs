...
#PITCH_THRESH = 0.12   # Z 軸縱傾絕對值超過此值才算偏上/偏下
#ROLL_THRESH  = 0.15   # 歪頭角度閾值（弧度，≈ 8.6°）；超過視為歪頭，套用穩定臉快取
YAW_THRESH   = 0.20   # 水平轉角絕對值超過此值才算偏左/偏右
PITCH_THRESH = 0.09   # Z 軸縱傾絕對值超過此值才算偏上/偏下
ROLL_THRESH  = 0.10   # 歪頭角度閾值（弧度，≈ 8.6°）；超過視為歪頭，套用穩定臉快取
