...
class SvmClassifier:
    ...
    def predict(self, X: np.ndarray,
        ...
        PoseLabels      : list[str]，可選，每個樣本對應的姿態字串（供 debug print 顯示）
        ...
        for i, x in enumerate(X):
            ...
            PoseTag   = PoseLabels[i] if PoseLabels and i < len(PoseLabels) else ""
            BaseTag   = f"[OCSVM/{self._Label}]" if self._Label else "[OCSVM]"
            Tag       = f"{BaseTag}[{PoseTag}]" if PoseTag else BaseTag
