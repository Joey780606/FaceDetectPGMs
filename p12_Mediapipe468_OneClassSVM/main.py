"""
main.py  (p12 OneClassSVM 版)

使用 MediaPipe FaceMesh + OneClassSVM 進行人臉辨識。
...
from svm_classifier_np import SVM_CONF_THRESH, SVM_MARGIN_THRESH, COSINE_VERIFY_THRESH
...
class MainApp(customtkinter.CTk):
    """主應用程式視窗，整合 webcam 與 OneClassSVM 人臉辨識。"""
    ...
    def _BuildUI(self, mode) -> None:
        ...
        self.title("人臉辨識系統（MediaPipe OneClassSVM）")
        ...
        # 信心度閾值：OneClassSVM raw decision score，範圍 -1.0 ~ 1.0
    ...
    def _isSameFace(BboxA: tuple, BboxB: tuple) -> bool:
        ...
        # 中心點距離 fallback（歪頭時 IoU 易低估，但中心不動）
        CxA = (LeftA + RightA) / 2
        CyA = (TopA  + BottomA) / 2
        CxB = (LeftB + RightB) / 2
        CyB = (TopB  + BottomB) / 2
        ...
        IsSame = (Dist / FaceSize) < STABLE_FACE_CENTER_THRESH
        print(f"[MainApp] StealEat IoU={Iou:.2f} centerDist={Dist/FaceSize:.2f} → {'沿用' if IsSame else '放棄'}快取")
        return IsSame
    ...
    def _OnBtnImport(self) -> None:
        ...
        if self._LearnActive:
            MsgBox.showwarning("訓練中", "請先等待目前學習完成再執行匯入。")
            return
