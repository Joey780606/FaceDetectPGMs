...
使用 MediaPipe FaceLandmarker（468 個 3D 特徵點）+ OpenCV LBPH 進行人臉辨識。
以 5 點仿射對齊（左眼、右眼、鼻尖、左嘴角、右嘴角）正規化人臉影像後交給 LBPH。
...
class MainApp(customtkinter.CTk):
    """主應用程式視窗，整合 webcam 與 MediaPipe 468 + LBPH 人臉辨識功能。"""
    ...
    def __init__(self):
        ...
        self._LastLearnKeyPoints = []   # list of {"left_eye":(cx,cy), ...}

        # UI 圖像參照（防止被 GC 回收）
        self._CurrentPhotoImage = None

        # 核心元件（FaceRecognizer 在背景執行緒初始化，先設為 None）
        ...
        # 初始化元件（開啟攝影機、背景載入 MediaPipe 與 LBPH 模型）
    ...
    def _BuildUI(self) -> None:
        ...
        self.title("人臉辨識系統（MediaPipe 468 + LBPH）")
        self.protocol("WM_DELETE_WINDOW", self._OnClose)
        self.resizable(True, True)

        # 使用 grid 佈局，讓 Row3（webcam）可垂直延伸
        ...
        customtkinter.CTkLabel(
            Row2Thresh, text="LBPH 距離閾值", anchor="w"
        ).grid(row=0, column=0, sticky="w", padx=(8, 4), pady=8)

        self._SldLbph = customtkinter.CTkSlider(
            Row2Thresh,
            from_=LBPH_THRESHOLD_MIN,
            to=LBPH_THRESHOLD_MAX,
            number_of_steps=180,
            command=self._OnLbphThreshChanged
        )
        self._SldLbph.set(LBPH_DEFAULT_THRESHOLD)
        self._SldLbph.grid(row=0, column=1, sticky="ew", padx=4, pady=8)

        self._LblLbphVal = customtkinter.CTkLabel(
            Row2Thresh,
            text=f"{LBPH_DEFAULT_THRESHOLD:.0f}",
            width=44,
            anchor="e"
        )
        self._LblLbphVal.grid(row=0, column=2, padx=(4, 8), pady=8)

        customtkinter.CTkLabel(
            Row2Thresh,
            text="（值越小越嚴格，Unknown 越多）",
            text_color="gray",
        ...
        Row3.grid(row=3, column=0, sticky="nsew", padx=10, pady=5)
        ...
        # ── Row 4：學習資料摘要 Log ───────────────────────────────────────
        Row4 = customtkinter.CTkFrame(self)
        Row4.grid(row=4, column=0, sticky="ew", padx=10, pady=(0, 10))
        Row4.grid_columnconfigure(0, weight=1)

        self._TxtLog = customtkinter.CTkTextbox(Row4, height=120, state="disabled")
    ...
    def _InitComponents(self) -> None:
        """初始化攝影機，並以背景執行緒載入 MediaPipe 與 LBPH 模型。"""
        # 1. 開啟攝影機（快速）
        ...
        # 4. 背景執行緒：建立 FaceRecognizer（含 MediaPipe 模型下載）+ LoadModel
        def InitWorker():
            try:
    ...
    def _OnBtnDetectNone(self) -> None:
        ...
        # 確認有已登錄的人臉資料
        if self._Recognizer is None or not self._Recognizer.CanDetect():
    ...
    def _StopLearning(self) -> None:
        ...
        # 訓練期間停用所有功能按鈕
        self._SetButtonsEnabled(False)
        self._AppendLog(f"[{self._LearnName}] 樣本收集完畢，正在訓練 LBPH 分類器...")
        ...
        if not self._InferenceActive:
            try:
                ...
                if Ok and Frame is not None:
                    ...
                    def Worker():
                        try:
                            # Retrain=False：學習期間只收集樣本，不重訓（學習結束後統一重訓）
    ...
    # LBPH 閾值 Slider 回調
    # --------------------------------------------------------------------------
    def _OnLbphThreshChanged(self, Value: float) -> None:
        """LBPH 距離閾值 Slider 拖動時，即時更新顯示值與辨識器閾值。"""
        self._LblLbphVal.configure(text=f"{Value:.0f}")
        if self._Recognizer is not None:
            self._Recognizer.SetThresholds(LbphThresh=Value)
