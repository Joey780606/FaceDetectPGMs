...
使用 MediaPipe FaceLandmarker + OpenCV FaceRecognizerSF 進行人臉辨識。
透過 CustomTkinter 建立 GUI，可學習並辨識不同人的身份。

架構：
    MediaPipe (偵測 + 對齊 112×112)
        → FaceRecognizerSF (128D 嵌入向量)
            → Cosine Similarity (辨識)
...
DETECT_NONE_DETECT_TARGET = 5      # DetectNone 偵測階段目標 frame 數（多數決）
...
class MainApp(customtkinter.CTk):
    ...
    def __init__(self, mode="normal", pet=None):
        ...
        self._LastLearnKeyPoints = []  # list of {"left_eye":(cx,cy), "right_eye":..., ...}
    ...
    def _BuildUI(self, mode) -> None:
        ...
        self.title("人臉辨識系統（MediaPipe + FaceRecognizerSF）")
    ...
    def _InitComponents(self) -> None:
        """初始化模型下載、攝影機與人臉辨識模型。"""
        # 1. 確認模型檔案存在（首次執行時自動下載）
        try:
            ModelsOk = ensureModels()
            if not ModelsOk:
                MsgBox.showwarning(
                    "模型下載失敗",
                    "部分模型檔案下載失敗，請確認網路連線後重新啟動。\n程式將以有限功能繼續執行。"
                )
        except Exception as Error:
            print(f"[MainApp] 模型下載異常：{Error}")
    ...
    def _StartLearning(self, PersonName: str) -> None:
        ...
        self._LblRemain.configure(
            text=f"Remaining study seconds: {LEARN_TIMEOUT_SECONDS}  ← 請慢慢左右、上下轉動頭部，以提升辨識多樣性"
        )
    ...
    def _StopLearning(self) -> None:
        ...
        # FaceRecognizerSF 不需重訓，呼叫 FinishLearning() 為空操作
        ...
        if SaveOk:
            Msg = (
                ...
                f"本次學習 {self._LearnFrameCount} 張\n\n"
    ...
    def _OnClose(self) -> None:
        ...
        self._LearnActive      = False
        self._Webcam.Close()
        # 釋放 MediaPipe FaceLandmarker 資源
        try:
            self._Recognizer._Aligner.close()
        except Exception:
            pass
        self.destroy()
