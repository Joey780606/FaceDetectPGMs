...
FaceRecognizer 類別：整合 MpFaceLandmarker、face_feature_3d 與 cosine_matcher_np，
提供學習（AddSample）、辨識（Predict）、儲存/載入（SaveModel/LoadModel）等功能。

辨識策略（Cosine 比對）：
  - 訓練時：計算每人的平均 1404 維特徵向量並 L2 正規化。
  - 推論時：計算 Cosine 相似度，最大值超過閾值者為此人，否則為 Unknown。
  - 1 人或多人皆使用同一套邏輯，不需特殊處理。

模型資料儲存至 face_model.npz（純 NumPy 壓縮格式，無 sklearn/scipy 依賴）。
訓練速度極快（只計算平均），不需 pickle 快取。
"""
...
from cosine_matcher_np import CosineMatcher, COSINE_UNKNOWN_THRESH
...
class FaceRecognizer:
    ...
    萃取 351 維臉部比例特徵向量後交由 CosineMatcher 進行 k-NN Cosine 比對。
    """
    ...
    def _trainMatcher(self) -> None:
        """
        根據目前的 _Samples 重新計算 CosineMatcher（各人平均向量）。
        速度極快，1人或多人均使用同一套邏輯。
        """
        try:
            ValidPersons = {Name: Vecs for Name, Vecs in self._Samples.items() if Vecs}
            ...
            print(f"[FaceRecognizer] 計算 CosineMatcher 失敗：{Error}")
            self._Matcher   = None
            self._IsTrained = False
