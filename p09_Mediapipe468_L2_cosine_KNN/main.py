...
from cosine_matcher_np import COSINE_UNKNOWN_THRESH
...
class MainApp(customtkinter.CTk):
    ...
    def _BuildUI(self) -> None:
        ...
        self.title("人臉辨識系統（MediaPipe 468 + Cosine 比對）")
        ...
        customtkinter.CTkLabel(Row2Thresh, text="Cosine 相似度閾值", anchor="w").grid(
        ...
        self._SldCosine.set(COSINE_UNKNOWN_THRESH)
        self._SldCosine.grid(row=0, column=1, sticky="ew", padx=4, pady=8)
        self._LblCosineVal = customtkinter.CTkLabel(
            Row2Thresh, text=f"{COSINE_UNKNOWN_THRESH:.2f}", width=44, anchor="e"
        )
    ...
    def _StopLearning(self) -> None:
        ...
        # 訓練期間停用所有功能按鈕，Log 提示
        self._SetButtonsEnabled(False)
        self._AppendLog(f"[{self._LearnName}] 樣本收集完畢，正在計算 Cosine 比對器...")
